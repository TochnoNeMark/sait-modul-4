...
/* ---- Пасхалка: ~1% шанс «потеряться» и попасть на 404 при заходе на любую страницу ----
   Срабатывает при загрузке (т.е. на каждом переходе между страницами). Сама 404 не
   подключает main.js, поэтому исключена; replace() не оставляет след в истории. */
(() => {
  const page = (location.pathname.split("/").pop() || "index.html").toLowerCase();
  if (page !== "404.html" && Math.random() < 0.01) {
